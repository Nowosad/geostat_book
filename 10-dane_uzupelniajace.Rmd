...
Kriging stratyfikowany zakłada, że zmienność badanego zjawiska zależy od zmiennej jakościowej (kategoryzowanej). Przykładowo, wartość badanej zmiennej jest różna w zależności od pokrycia terenu. Kriging stratyfikowany wymaga posiadania danych zmiennej jakościowej (kategoryzowanej) na całym badanym obszarze.
...
Następnie dla każdego obszaru przeprowadzona jest niezależna estymacja wartości analizowanej cechy. Należy jednynie wcześniej zadbać, by w siatce nie było elementów `NA` dotyczących zmiennych jakościowych. W przykładzie tworzona jest nowa siatka (`siatka2`) nie zawierająca braków wartości dla zmiennej `clc`.
...
Ostatnim krokiem jest estymacja geostatystyczna, w której oprócz czterech podstawowych argumentów, definiujemy także parametr `beta`. Jest to w tym wypadku obiekt uzyskany na podstawie regresji liniowej.
