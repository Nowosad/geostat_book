...
Centrowanie danych polega na wybraniu przeciętnej wartości predyktora, a następnie odjęciu wybranej wcześniej wartości od wszystkich wartości predyktorów 
...
Skalowanie danych to dzielenie każdej wartości predyktora przez jego odchylenie standardowe (rycina \@ref(fig:03-eksp-analiza-danych-24)).
...
Do standaryzacji (centrowanie i skalowanie) służy funkcja `scale()` (rycina \@ref(fig:03-eksp-analiza-danych-26)).
...
Transformację logarytmiczną można odwrócić używając funkcji `exp()` (rycina \@ref(fig:03-eksp-analiza-danych-28)).
...
Odwrócenie tej transformacji odbywa się poprzez podniesienie wartości do potęgi (`^2`) (rycina \@ref(fig:03-eksp-analiza-danych-30)).
