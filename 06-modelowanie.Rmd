...
Estymacje i symulacje przestrzenne wymagają modelu struktury przestrzennej analizowanej cechy, a nie tylko wartości empirycznych (wyliczonych z danych). Dodatkowo, matematycznie modelowanie wygładza chaotyczne fluktuacje danych empirycznych.
...
Dodatkowo, można jest porównać na jedym wykresie poprzez argument `as.groups = TRUE`.
...
```{r, echo=FALSE}
knitr::include_graphics('figs/eyefit.png')
```
...
2. Zdefiniować parametry semiwariogramu, tj. semiwariancja cząstkowa (`psill`), skrócona nazwa używanej funkcji (`model`) oraz jej zasięg (`range`) w funkcji `vgm()`. Uzyskany model można przedstawić w funkcji `plot()` podając nazwę obiektu zawierającego semiwariogram epiryczny oraz obiektu zawierającego model
...
Model potęgowy (`Pow`) to przykład tzw. modelu nieograniczonego. Jego wartość rośnie w nieskończoność, dlatego niemożliwe jest określenie jego zasięgu. W przypadku modelu potęgowego, paramter `range` oznacza wykładnik potęgowy.
