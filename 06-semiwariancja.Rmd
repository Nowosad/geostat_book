...
- W efekcie otrzymujemy miarę podobieństwa pomiędzy wartością głowy i ogona (rycina \@ref(fig:z_przs0))

```{r z_przs0, echo=FALSE, fig.cap = "Relacja między wartościami zmiennej temp w danym miejscu a wartościami zmiennej temp w odległości do 200 metrów."}
...
Taki wykres można stworzyć używając funkcji `hscat()` z pakietu **gstat** (rycina \@ref(fig:z_przs)). 
Przykładowo, na poniższym wykresie widać wartość cechy `temp` z kolejnymi przesunięciami - od 0 do 500 metrów, od 500 metrów do 1000 metrów, itd.
...
```{r z_przs, fig.height=7, fig.cap = "Wykres rozrzutu z przesunięciem dla zamiennej temp."}
...
Czy badane zjawisko wykazuje izotropia czy anizotropia?
