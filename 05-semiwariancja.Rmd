...
```{r }
kowario <- variogram(temp~1, locations = punkty, covariogram = TRUE)
plot(kowario)
```
...
```{r }
vario_cloud <- variogram(temp~1, locations = punkty, cloud = TRUE)
plot(vario_cloud)
```
...
vario_cloud_sel <- plot(variogram(temp~1, locations = punkty, cloud = TRUE), digitize = TRUE)
```
...
```{r }
pow_pr <- area(granica) / nrow(punkty)
pow_pr
```
...
```{r}
vario_par <- variogram(temp~1, locations = punkty, cutoff = 4000)
plot(vario_par)
```
...
```{r }
vario_par <- variogram(temp~1, locations = punkty, cutoff = 1000, width = 250)
plot(vario_par)
```
...
```{r }
vario_map <- variogram(temp~1, locations = punkty, cutoff = 4000, width = 400, map = TRUE)
...
```{r }
vario_kier <- variogram(temp~1, locations = punkty, alpha = c(0, 45, 90, 135))
plot(vario_kier)
```
