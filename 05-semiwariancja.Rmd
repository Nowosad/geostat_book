...
hscat(temp~1, as(punkty, "Spatial"), breaks = c(0, 200))
```
...
hscat(temp~1, as(punkty, "Spatial"), breaks = seq(0, 4000, by = 500))
```
...
```{r }
kowario <- variogram(temp~1, as(punkty, "Spatial"), covariogram = TRUE)
plot(kowario)
```
...
```{r }
odl <- dist(st_coordinates(punkty)[c(1, 2), ])
...
```{r }
vario_cloud <- variogram(temp~1, as(punkty, "Spatial"), cloud = TRUE)
plot(vario_cloud)
```
...
vario_cloud_sel <- plot(variogram(temp~1, as(punkty, "Spatial"), cloud = TRUE), digitize = TRUE)
```
...
plot(vario_cloud_sel, as(punkty, "Spatial"))
```
...
```{r }
vario_par <- variogram(temp~1, as(punkty, "Spatial"))
vario_par
```
...
```{r}
vario_par <- variogram(temp~1, as(punkty, "Spatial"), cutoff = 4000)
plot(vario_par)
```
...
```{r }
vario_par <- variogram(temp~1, as(punkty, "Spatial"), cutoff = 1000, width = 250)
plot(vario_par)
```
...
```{r }
vario_map <- variogram(temp~1, as(punkty, "Spatial"), cutoff = 4000, width = 400, map = TRUE)
...
```{r }
vario_kier <- variogram(temp~1, as(punkty, "Spatial"), alpha = c(0, 45, 90, 135))
plot(vario_kier)
```
