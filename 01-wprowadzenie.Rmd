...
Więcej o pakiecie **sf** można przeczytać w appendiksie \@ref(r-spatial).
...
- Okreslane są w miarach liniowych (np. metrach)
<!-- -->
...
```{r}
library(rgdal)
granica <- readOGR(dsn = "dane/granica.gpkg")
plot(granica)
```
...
# formaty wektorowe
writeOGR(granica, dsn = "dane", layer = "nowa_granica", driver = "ESRI Shapefile")
```
...
writeOGR(granica, dsn = "dane/nowa_granica.gpkg", layer = "granica", driver = "GPKG")
```
