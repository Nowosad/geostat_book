...
par(mfrow = c(1, 2))
plot(sk["var1.pred"], key.pos = NULL, reset = FALSE, main = "Estymacja SK")
plot(sk["var1.var"], key.pos = NULL, main = "Wariancja estymacji SK")
```
...
par(mfrow = c(1, 2))
plot(ok["var1.pred"], key.pos = NULL, reset = FALSE, main = "Estymacja OK")
plot(ok["var1.var"], key.pos = NULL, main = "Wariancja estymacji OK")
```
...
W pierwszym kroku konieczne jest dodanie współrzęnych do używanego obiektu, jak też i do używanej siatki.
...
Następnie pierwszy z argumentów w funkcji `variogram()` musi przyjąć postać `temp~x+y`, co oznacza, że uwzględniamy liniowy trend zależny od współrzędnej `x` oraz `y`.
...
Należy tutaj pamiętać, aby wzór (w przykładzie `temp~x + y`) był taki sam podczas budowania semiwariogramu, jak i estymacji.
...
par(mfrow = c(1, 2))
plot(kzt["var1.pred"], key.pos = NULL, reset = FALSE, main = "Estymacja KZT")
plot(kzt["var1.var"], key.pos = NULL, main = "Wariancja estymacji KZT")
```
