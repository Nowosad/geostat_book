...
```{r }
voronoi_interp <- voronoi(as(punkty, "Spatial")) %>% 
        st_as_sf()
plot(st_geometry(voronoi_interp), main = "Poligony Voronoia")
...
```{r }
projection(siatka) <- st_crs(punkty)$proj4string
punkty_sp <- as(punkty, "Spatial")
idw_wolin <- idw(temp~1, punkty_sp, siatka, idp = 2)
idw_wolin_r <- raster(idw_wolin)
plot(idw_wolin_r, main = "IDW")
contour(idw_wolin_r, add = TRUE)
```
...
```{r }
# wielomian 1 stopnia
wielomian_1 <- gstat(formula = temp~1, data = punkty_sp, degree = 1)
wielomian_1_pred <- predict(wielomian_1, newdata = siatka)
wielomian_1_pred_r <- raster(wielomian_1_pred)
plot(wielomian_1_pred_r, main = "Powierzchnia trendu - wielomian pierwszego stopnia")
contour(wielomian_1_pred_r, add = TRUE)
# wielomian 2 stopnia
wielomian_2 <- gstat(formula = temp~1, data = punkty_sp, degree = 2)
wielomian_2_pred <- predict(wielomian_2, newdata = siatka)
wielomian_2_pred_r <- raster(wielomian_2_pred)
plot(wielomian_2_pred_r, main = "Powierzchnia trendu - wielomian drugiego stopnia")
contour(wielomian_2_pred_r, add = TRUE)
# wielomian 3 stopnia
wielomian_3 <- gstat(formula = temp~1, data = punkty_sp, degree = 3)
wielomian_3_pred <- predict(wielomian_3, newdata = siatka)
wielomian_3_pred_r <- raster(wielomian_3_pred)
plot(wielomian_3_pred_r, main = "Powierzchnia trendu - wielomian trzeciego stopnia")
contour(wielomian_3_pred_r, add = TRUE)
```
...
plot(spline, main = "Funkcje sklejane")
contour(spline, add = TRUE)
```
