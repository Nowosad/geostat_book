...
ggplot(df_idw, aes(x = distance, y = value, color = as.factor(key), group = key)) +
        geom_path(size = 2) + 
        scale_color_brewer(type = "qual", name = "Inverse distance\nweighting power") +
...
spplot(siatka, "tps_pred", contour = TRUE, main = "Funkcje sklejane")
```
...
p2 <- spplot(siatka, main = "Funkcje sklejane")
...
1. Korzystając z danych XX wykonaj interpolację używając:
- Poligonów Voronoi'a
- Metody IDW
- Funkcji wielomianowych
- Funckji sklejanych

2. Porównaj uzyskane wyniki poprzez ich wizualizację.
