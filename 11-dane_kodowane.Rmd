...
```{r 11-dane-kodowane-5, fig.cap="Semiwariogram empiryczny binarnej zmiennej temp_ind."}
vario_ind = variogram(temp_ind~1, locations = punkty)
plot(vario_ind)
```

```{r 11-dane-kodowane-6, fig.cap = "Model semiwariogramu empirycznego binarnej zmiennej temp_ind."}
...
```{r plotsy2ok, echo=FALSE, fig.height=8, fig.cap = "Estymacja i wariancja estymacji używając metody krigingu danych kodowanych (IK)."}
library(gridExtra)
p1 = p1_base + geom_stars(data = ik["var1.pred"]) +
        ggtitle("Prawdopodobieństwo Temp > 20")
p2 = p1_base + geom_stars(data = ik["var1.var"]) + 
        ggtitle("Wariancja estymacji IK")
...
```{r plotsy2ik2, echo=FALSE, fig.height=8, message=FALSE, fig.cap = "Binarne mapy dla progu ustalonego na 0.1 oraz 0.75."}
library(gridExtra)
p1p = p1_base + geom_stars(data = ik["prog1"]) +
        ggtitle("Próg: 0.1") + scale_fill_carto_d(palette = "Safe")
p2p = p1_base + geom_stars(data = ik["prog2"]) +
        ggtitle("Próg: 0.75") + scale_fill_carto_d(palette = "Safe")
...
Na poniższych przykładach w ten sposób ustalono trzy progi - poniżej 20, poniżej 16, oraz poniżej 12 stopni Celsjusza (rycina \@ref(fig:ploty_trzyik)).
...
```{r ploty_trzyik, echo=FALSE, fig.height=12, fig.cap = "Estymacje używając metody krigingu danych kodowanych (IK) dla różnych progów prawdopodobieństwa."}
library(gridExtra)
p1i = p1_base + geom_stars(data = ik20["var1.pred"]) +
        ggtitle("Prawdopodobieństwo Temp < 20")
p2i = p1_base + geom_stars(data = ik16["var1.pred"]) + 
        ggtitle("Prawdopodobieństwo Temp < 16")
p3i = p1_base + geom_stars(data = ik12["var1.pred"]) + 
        ggtitle("Prawdopodobieństwo Temp < 12")
