...
Posiadając dopasowane modele oraz siatkę można uzyskać wynik używając funkcji `predict()` (rycina \@ref(fig:kokriging_predicted)). 
...
```{r kokriging_predicted, echo=FALSE, fig.height=8, fig.cap = "Estymacja i wariancja estymacji używając metody kokrigingu (CK)."}
library(gridExtra)
p1 = p1_base + geom_stars(data = ck["SAVI.pred"]) +
        ggtitle("Estymacja CK")
p2 = p1_base + geom_stars(data = ck["SAVI.var"]) + 
        ggtitle("Wariancja estymacji CK")
...
Na jego podstawie wydziel dwa obiekty - `meuse155` zawierający tylko zmienną `lead` dla 164 punktów, oraz `meuse60` zawierający tylko zmienną `copper` dla 60 punktów.
