...
$$ R^2 = \frac{\sum_{i=1}^{n} (\hat v_t - \overline{v})^2}{\sum_{i=1}^{n} (v_t - \overline{v})^2} $$
...
test_sk <- krige(temp~1, train, newdata = test, 
                 model = fitted, beta = 15.9, debug.level = 0)
blad_predykcji_sk <- test$temp - test_sk$var1.pred
test_sk$blad_predykcji_sk <- blad_predykcji_sk
test_sk$true <- test$temp

cuts <- c(-5, -3, -1, 1, 3, 5)
spplot(test_sk, "blad_predykcji_sk", main = "Błąd predykcji", cuts = cuts)
```
...
```{r hist, echo=FALSE}
ggplot(as.data.frame(test_sk), aes(blad_predykcji_sk)) +
    geom_histogram() + xlab("Błąd predykcji") + 
    ylab("Liczebność")
```
...
test_sk$blad_predykcji_sk <- blad_predykcji_sk
spplot(test_sk, "blad_predykcji_sk", cuts = c(-5, -3, -1, 1, 3, 5))
