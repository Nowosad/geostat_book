...
Określenie granic obszaru można wykonać na podstawie zasięgu danych punktowych za pomocą funkcji `bbox()` z pakietu **sp**.
...
Przy ustalaniu wartości początkowej i końcowej konieczne jest ich rozszerzenie względem wartości z funkcji `bbox()`, aby wszystkie analizowane punkty znalazły się na badanym obszarze. 
...
Sprawdzenie, czy uzyskana siatka oraz dane punktowe się na siebie nakładają można wykonać za pomocą funkcji `plot()`. 
...
W tych modelach brakuje jednak szacunków na temat oceny błędu modelu.
...
p4 <- spplot(idw_interp, "var1.pred", main = "IDW")
