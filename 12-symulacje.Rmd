...
sym_bezw1 <- krige(formula = z~1, 
                   model = vgm(psill = 0.025, model = "Exp", range = 100), 
                   newdata = siatka, 
                   beta = 1,
                   nmax = 30, 
                   locations = NULL, 
...
Prywrócenie oryginalnej jednostki odbywa się poprzez argumenty `MARGIN = 2` oraz `FUN = exp`.
...
sym_sk@data <- data.frame(apply(sym_sk@data, MARGIN = 2, FUN = exp))
```
...
sym_sk@data <- data.frame(apply(sym_sk@data, MARGIN = 1, FUN = sd))
```
...
plot(vario_ind)
fitted_ind <- fit.variogram(vario_ind, vgm(model = "Sph", nugget = 0.3))
fitted_ind
...
<!-- 1. Symulacja bezwarunkowa -->
<!-- 2. Symulacja warunkowa -->
<!-- 3. Symulacja danych kodowanych -->
