...
Dane przestrzenne mogą być reprezentowane poprzez szereg różnych klas obiektów z użyciem różnych pakietów R. 
...
Więcej o pakiecie **sf** oraz **raster** można przeczytać w [rozdziale drugim](https://geocompr.robinlovelace.net/spatial-class.html) książki Geocomputation with R.

### GDAL/OGR

- Pakiety **rgdal**, **raster** czy **sf** wykorzystują biblioteki [GDAL/OGR](http://www.gdal.org/) w R do wczytywania i zapisywania danych przestrzennych
...
- Pakiety **sp**, **raster**, czy **sf** używają biblioteki [PROJ](https://proj4.org/) do określania i konwersji układów współrzędnych
...
- Pakiety **rgeos** czy **sf** używają biblioteki [GEOS](http://geos.osgeo.org/) do wykonywania operacji przestrzennych
...
Dane wektorowe (np. GeoPackage czy ESRI Shapefile) można odczytać za pomocą funkcji `readOGR()` z pakietu **rgdal**. 
W najprostszej sytuacji przyjmuje ona jeden argument `dsn`, który oczekuje ścieżki do pliku wektorowego.
...
Poniżej pokazane są przykłady kilku najprostszych funkcji - `plot()` oraz `spplot()` z pakietu `sp`.
