...
dane_punktowe_sf2 = st_join(dane_punktowe_sf, st_as_sf(siatka_stars))
```
...
Opiera się to o kilka indeksów, np. dla obiektu `siatka_stars` są to `[indeks atrybutu, indeksy kolumn, indeksy wierszy, indeksy kanału]`.
Przykładowo indeks `[1, , , 4]` pozwala na wyświetlenie kanału czwartego z pierwszego atrybutu.

```{r}
# wybrany kanał
plot(siatka_stars[1, , , 4])
```

Indeksowanie pozwala też na wyświetlenie fragmentu danych rastrowych.
Na przykład, indeks `[1, 2:50, 1:20, ]` pozwala na wyświetlenie wszystkich warstw, ale tylko dla kolumn od 20 do 50 i wierszy od 1 do 20.

```{r}
# wybrany zasięg przestrzenny
plot(siatka_stars[1, 20:50, 1:20, ])
```
