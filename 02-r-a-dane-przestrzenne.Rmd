...
<!--
Dane wektorowe (np. ESRI Shapefile) można odczytać za pomocą funkcji `readOGR()` z pakietu **rgdal**. 
Dla danych w formacie ESRI Shapefile, przyjmuje ona co najmniej dwa argumenty - `dsn` oraz `layer`. 
...
Zapisywanie w bazach danych przestrzennych odbywa się poprzez nazwy zapisywanego obiektu (np. `granica`), ścieżki do zapisywanego pliku (np. `dane/nowa_granica.gpkg`), nazwę dla nowo utworzonej warstwy (np. `granica`), oraz sterownik - w przypadku danych GeoPackage jest to `GPKG`.
