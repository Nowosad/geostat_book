...
- GIS - **sf**, **stars**, **raster**, **terra**, **sp**, **rgdal**, **rgeos**
...
- Pakiety **rgdal**, **raster**, **stars**, czy **sf** wykorzystują biblioteki GDAL/OGR w R do wczytywania i zapisywania danych przestrzennych
- http://www.gdal.org/
...
- Pakiety **sp**, **raster**, **stars**, czy **sf** używają biblioteki PROJ do określania i konwersji układów współrzędnych
- https://proj.org/
...
Proste wizualizacje uzyskanych danych klasy przestrzennej, np. `sf`, można uzyskać za pomocą funkcji `plot()` (rycina \@ref(fig:02-r-a-dane-przestrzenne-5) i \@ref(fig:02-r-a-dane-przestrzenne-7a)).
...
Obiekt `siatka_stars` posiada dwa wymiary (`x` i `y`) oraz jeden atrybut (`siatka.tif`).

```{r 02-r-a-dane-przestrzenne-10a}
siatka_stars
```

Jest zbudowany jako lista zawierająca dwuwymiarową macierz wraz ze zbiorem metadanych (można to sprawdzić za pomocą funkcji `str(siatka_stars)`).
...
st_write(granica_sf, dsn = "dane/granica.gpkg")
```
...
Domyślnie wyświetla ona szereg map - po jeden dla każdej zmiennej (rycina \@ref(fig:02-r-a-dane-przestrzenne-16)).
