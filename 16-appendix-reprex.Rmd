...
- Z małego zbioru danych wystarczający do odtworzenia obliczeń
- Krótkiego kodu, który może być uruchomiony na powyższym zbiorze danych
- Czasem ważne są informacje o używanej wersji R, posiadanym systemie operacyjnym, wersjach używanych pakietów, etc. 
...
Po jego uruchomieniu otrzymujemy wynik zapisany w schowku jako Markdownw oraz w postaci wyświetlonego HTML:
...
Odpowiedź - ten kod nie jest w pełni samowystarczający - brakuje tam dołączenia pakietu **geostatbook**, który zawiera zbiór danych `punkty`.
Drugi problem to użyta zmienna do wizualizacji - zmienna `"temperatura"` nie istnieje w zbiorze danych `punkty`. 
